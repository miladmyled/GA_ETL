...
def main():
    ...
    for i in range((limit_date - ref_date).days - 1):
        ...
        data = faq.fetch_data(VIEW_ID, analytics, step_time, 'events')
        data.columns = ['date', 'eventAction', 'eventCategory', 'eventLabel', 'totalEvents', 'uniqueEvents']
        data['date'] = pd.to_datetime(data['date'])
        data = data.drop(['eventCategory'], axis=1)

        try:
            cursor.fast_executemany = True
            sql_comm = '''INSERT INTO [{}].[dbo].[{}]
            ([date],[eventAction],[eventLabel],[totalEvents],[uniqueEvents])
