                ...
                'reportRequests': [
                    {
                        ...
                        'dimensions': [{"name": "ga:date"}, {"name": "ga:productListName"}],
...
def fetch_data(_id, analytics, input_date):
    response = get_report(_id, analytics, input_date, input_date, '0')
    ...
    while (len(df.index) == BATCH_SIZE):
        idx += BATCH_SIZE
        response = get_report(_id, analytics , input_date, input_date, str(idx))
        df = ga_engine.create_df(response)
        dataframes.append(df)
